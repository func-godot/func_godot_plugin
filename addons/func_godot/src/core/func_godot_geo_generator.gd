...
func run() -> void:
	...
	for i in range(map_data.entity_geo.size()):
		map_data.entity_geo[i] = FuncGodotMapData.EntityGeometry.new()
	...
	for e in range(map_data.entities.size()):
		...
		for i in range(entity_geo.brushes.size()):
			entity_geo.brushes[i] = FuncGodotMapData.BrushGeometry.new()
		...
		for b in range(entity.brushes.size()):
			...
			for i in range(brush_geo.faces.size()):
				brush_geo.faces[i] = FuncGodotMapData.FaceGeometry.new()
	...
	for f0 in range(face_count):
		...
		for f1 in range(face_count):
			for f2 in range(face_count):
				...
				if duplicate_idx < 0:
					var new_face_vert:= FuncGodotMapData.FaceVertex.new()
...
func intersect_faces(f0: FuncGodotMapData.Face, f1: FuncGodotMapData.Face, f2: FuncGodotMapData.Face):
...
func vertex_in_hull(faces: Array[FuncGodotMapData.Face], vertex: Vector3) -> bool:
	for face in faces:
...
func get_standard_uv(vertex: Vector3, face: FuncGodotMapData.Face, texture_width: int, texture_height: int) -> Vector2:
	var uv_out: Vector2
...
func get_valve_uv(vertex: Vector3, face: FuncGodotMapData.Face, texture_width: int, texture_height: int) -> Vector2:
	var uv_out: Vector2
...
func get_standard_tangent(face: FuncGodotMapData.Face) -> Vector4:
...
func get_valve_tangent(face: FuncGodotMapData.Face) -> Vector4:
...
func get_entities() -> Array[FuncGodotMapData.EntityGeometry]:
